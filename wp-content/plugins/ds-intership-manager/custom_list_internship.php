...
<?php
    ...
    $max=1000;
    $start=($max*($pageNo-1));
    $totSql="SELECT Count(*) FROM ".enquery_details;
    $totalRec = $wpdb->get_row($totSql,ARRAY_N);

    $recCount=$totalRec[0];

	$querystr="SELECT * FROM ".enquery_details." ORDER BY ".enquery_details.".created_datetime DESC LIMIT $start,$max";
...
<div class="wrap">
    ...
    <h2>
        <?php echo "Enquiry Form List"; ?> &nbsp;<input type="button" id="btnExport" value="Export in Excel" />
    </h2>
     <?php if(isset($msg)) : ?>
            ...
            <tbody>
                <tr>
				   <th width="4%"><strong>Sr. No.</strong></th>
                    ...
                    <th width="8%"><strong>Contact No.</strong></th>
                    <th width="15%"><strong>E-mail ID</strong></th>
                    <th width="8%"><strong>Purpose</strong></th>  
                    <th width="10%"><strong>Highest Qualification</strong></th>
					<th width="10%"><strong>Program</strong></th>
                ...
                <?php $i=0; foreach($customPages as $cpage){ $i++; ?>
                    <tr id="custompage-<?php echo $cpage->id; ?>" class="alternate iedit">
					    <td><?php echo $i; ?></td>
                        <td><?php echo ucfirst($cpage->name); ?></td>
						<td><?php echo $cpage->contact_no; ?></td>
						<td><?php echo $cpage->email_id; ?></td>
						<td><?php echo $cpage->purpose; ?></td>
						<td><?php echo $cpage->high_quali; ?></td>
						<td><?php echo $cpage->programs; ?></td>
						<td><?php echo $cpage->address; ?></td> 
        ...
        $(function () {
            $("#btnExport").click(function () {
                $("#tblCustomers").table2excel({
                    filename: "Enquiry-Form-Report.xls"
                });
            });
        });
    </script>