...
<?php
    ...
    $max=1000;
    $start=($max*($pageNo-1));
    $totSql="SELECT Count(*) FROM ".student_grievance_details;
    $totalRec = $wpdb->get_row($totSql,ARRAY_N);

    $recCount=$totalRec[0];

	$querystr="SELECT * FROM ".student_grievance_details." ORDER BY ".student_grievance_details.".created_datetime DESC LIMIT $start,$max";
...
<div class="wrap">
    ...
    <h2>
        <?php echo "Student Form List"; ?> &nbsp;<input type="button" id="btnExport" value="Export in Excel" />
    </h2>
     <?php if(isset($msg)) : ?>
            ...
            <tbody>
                <tr>
				   <th width="4%"><strong>Sr. No.</strong></th>
                    ...
                    <th width="8%"><strong>Course</strong></th>
                    <th width="15%"><strong>Year</strong></th>
                    <th width="8%"><strong>Roll No.</strong></th>  
                ...
                <?php $i=0; foreach($customPages as $cpage){ $i++; ?>
                    <tr id="custompage-<?php echo $cpage->id; ?>" class="alternate iedit">
					    <td><?php echo $i; ?></td>
                        <td><?php echo ucfirst($cpage->student_name); ?></td>
						<td><?php echo $cpage->course; ?></td>
						<td><?php echo $cpage->year; ?></td>
						<td><?php echo $cpage->roll_no; ?></td>
        ...
        $(function () {
            $("#btnExport").click(function () {
                $("#tblCustomers").table2excel({
                    filename: "student-Form-Report.xls"
                });
            });
        });
    </script>